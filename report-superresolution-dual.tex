...
For the first term defined in Equation \ref{eq:yn1} we get the following derivation:
\begin{align*}
	y^{n+1} &= prox_{\sigma F^*}(y^n + \sigma K \bar{x}^n) \\
			&= y^n + \sigma K \bar{x}^n - 
				\sigma \cdot \prox_{K / \sigma}((y^n + \sigma K \bar{x}^n) / \sigma) 
				\label{eq:moreaus_identity_used} \\
			&= y^n + \sigma \nabla \bar{x}^n - 
				\sigma \cdot \prox_{\nabla / \sigma}((y^n + \sigma \nabla \bar{x}^n) / \sigma) \\
			&= \frac{y^n + \sigma \nabla \bar{x}^n}
				    {\max(1, \twonorm{y^n + \sigma \nabla \bar{x}^n})}
\end{align*}
Using Moreau's Identity in step \ref{eq:moreaus_identity_used} which is defined as
\begin{equation}
